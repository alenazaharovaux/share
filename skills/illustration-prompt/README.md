...
### Vehicle defaults (built-in)

When vehicles appear in scenes, the skill automatically adds rules that prevent common AI generator failures:
- **Windows transparent** (interior visible through glass)
- **Doors closed** (generators love to remove doors or render cutaway views)
- **Proportions balanced** (no stretched or compressed vehicles)

These rules are applied automatically in OBJECTS and NEGATIVE blocks.
...
### Дефолты для транспорта (встроенные)

При появлении машин в сцене скилл автоматически добавляет правила, предотвращающие типичные ошибки генераторов:
- **Окна прозрачные** (виден интерьер через стекло)
- **Двери закрыты** (генераторы любят убирать двери или делать cutaway)
- **Пропорции сбалансированы** (без растянутых или сжатых машин)
