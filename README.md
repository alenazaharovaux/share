...
| [Product Market Fit](skills/pmf/) | Full product-market fit cycle for one product — 10 stages from first hypothesis to post-launch metrics, resumable between sessions over months. Opinionated on Cagan, Bland (DVF), Helmer (7 Powers), Sean Ellis, First Round. Bilingual (EN + RU). By [Alena Zakharova](https://github.com/alenazaharovaux) (MIT) |
| [topic-clustering](skills/topic-clustering/) | Cluster large volumes of text into semantic topics with BERTopic, without a terminal. Handles Telegram exports, chat histories, transcripts, post dumps. Every topic is a real list of documents, so nothing is invented and coverage is not lost. Multilingual. Bilingual (EN + RU). By [Alena Zakharova](https://github.com/alenazaharovaux) (MIT) |
...
| [Product Market Fit](skills/pmf/) | Полный цикл product-market fit для одного продукта — 10 стадий от первой гипотезы до метрик после запуска, возобновляется между сессиями месяцами. Жёсткий по методологии: Каган, Бланд (DVF), Хелмер (7 Powers), Шон Эллис, First Round. Двуязычный (EN + RU). Автор: [Алена Захарова](https://github.com/alenazaharovaux) (MIT) |
| [topic-clustering](skills/topic-clustering/) | Кластеризует большие объёмы текста по смысловым темам через BERTopic, без терминала. Понимает экспорты Telegram, истории переписки, транскрипты, выгрузки постов. Каждая тема — реальный список документов, поэтому ничего не выдумывается и охват не теряется. Многоязычный. Двуязычный (EN + RU). Автор: [Алена Захарова](https://github.com/alenazaharovaux) (MIT) |
